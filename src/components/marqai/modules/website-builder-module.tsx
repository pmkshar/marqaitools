...
export function WebsiteBuilderModule() {
  ...
  async function generate() {
    ...
    try {
      ...
      // Surface fallback warnings so the user knows when the AI failed
      // and a template was substituted instead.
      if (data.source === "fallback" || data.warning) {
        toast.warning("Landing page generated (template mode)", {
          description: data.warning ?? "AI generation failed; showing template page instead.",
        });
      } else {
        toast.success("Landing page generated", {
          description: `${data.sections.length} sections · AI-generated`,
        });
      }

      // Auto-open the preview so the user immediately sees the result
      setPreview(asset);
    } catch (e) {
          ...
          <CardContent>
            {websites.length === 0 ? (
              ...
              <div className="space-y-3">
                {websites.map((w) => (
                  <div key={w.id} className="rounded-lg border p-4 hover:border-emerald-300 hover:ring-1 hover:ring-emerald-200 transition cursor-pointer" onClick={() => setPreview(w)}>
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                        ...
                        <Button size="sm" variant="outline" className="h-7" onClick={() => downloadHtml(w)} title="Download HTML">
