...
// ============================================================
// SEED BUGS — Bugzilla-style defect tracker seed data
// ============================================================
...
export const seedBugs: Bug[] = [
  {
    id: "bug-1",
    displayId: "BUG-1001",
    title: "AI Sales Agent qualifier returns unparseable JSON for short product context",
    description:
      "When the Qualifier agent is given a product context shorter than ~20 characters, the underlying LLM occasionally returns plain prose instead of the expected JSON object. The API then fails to parse the response and surfaces a 500 to the UI.",
    reproSteps: [
      "Navigate to AI Sales Agents → Agents tab.",
      "Open a Qualifier agent whose productContext is fewer than 20 characters.",
      "Click 'Qualify Lead' with any lead profile.",
      "Observe a 500 response from /api/marqai/sales/qualify.",
    ],
    expectedBehavior:
      "The API should always return a structured BANT/MEDDIC qualification object, falling back to a templated response when the LLM output is unparseable.",
    actualBehavior:
      "API returns HTTP 500 with body { error: 'Unexpected token < in JSON at position 0' } and the UI shows a generic 'Something went wrong' toast.",
    productType: "ai",
    module: "AI Sales Agents",
    environment: "production",
    environmentDetails: "Chrome 126 / macOS 14 / Vercel deployment",
    methodologyId: "ai-prompt-hallucination",
    methodologyCategory: "ai-strategies",
    severity: "major",
    priority: "P1",
    status: "assigned",
    resolution: "open",
    assigneeUserId: "user-vikram",
    assigneeName: "Vikram Shah",
    reporterUserId: "user-nikhil",
    reporterName: "Nikhil Rao",
    ccUserIds: ["user-priya"],
    tags: ["ai-output-validation", "qualifier", "json-parse"],
    externalIssueUrl: undefined,
    comments: [
      {
        id: "c-1",
        authorName: "Nikhil Rao",
        authorRole: "Sales Development Rep",
        body: "Reproduced twice today. Happens consistently for very short product contexts.",
        createdAt: "2026-07-04T09:15:00.000Z",
      },
      {
        id: "c-2",
        authorName: "Vikram Shah",
        authorRole: "AI QA Lead",
        body: "Confirmed. Looks like the LLM is treating short inputs as a 'greeting' and responding in prose. Will add a stronger system prompt + JSON-mode enforcement + fallback template.",
        changeKind: "status",
        changeFrom: "new",
        changeTo: "assigned",
      ...
      },
      {
        ...
        authorName: "Vikram Shah",
        authorRole: "AI QA Lead",
        body: "Assigning to myself. Will pair with Arjun on the prompt engineering side.",
        changeKind: "assignee",
        changeFrom: "Unassigned",
        changeTo: "Vikram Shah",
        createdAt: "2026-07-04T10:31:00.000Z",
      },
    ],
    attachments: [],
  ...
  },
  {
    id: "bug-2",
    displayId: "BUG-1002",
    title: "Leads Generator: email pattern regex rejects valid plus-addressed emails",
    description:
      "The Leads Generator applies a strict email regex that rejects plus-addressed emails (e.g. jane+spam@acme.com). These are valid per RFC 5233 and commonly used by enterprise prospects for filtering — we are flagging them as invalid.",
    reproSteps: [
      "Open Leads Generator module.",
      "Generate leads for a product targeting enterprise SaaS.",
      "Inspect the generated lead list — note any emails containing '+' are missing.",
    ],
    expectedBehavior:
      "Plus-addressed emails should be accepted and persisted. The validation regex must match RFC 5233.",
    actualBehavior:
      "Generated leads that would have had '+' in the email are silently dropped. No error surfaced.",
    productType: "ai",
    module: "Leads Generator",
    environment: "staging",
    environmentDetails: "All modern browsers",
    methodologyId: "data-validation",
    methodologyCategory: "ai-methodologies",
    severity: "normal",
    priority: "P2",
    status: "new",
    resolution: "open",
    reporterUserId: "user-arjun",
    reporterName: "Arjun Reddy",
    tags: ["email-validation", "leads", "rfc-5233"],
    comments: [
      {
        id: "c-4",
        authorName: "Arjun Reddy",
        authorRole: "Head of Marketing",
        body: "Spotted during sprint demo. Lost 4 enterprise leads this week.",
  ...
  },
  {
    id: "bug-3",
    displayId: "BUG-1003",
    title: "Billing: Stripe webhook signature verification fails on Vercel edge runtime",
    description:
      "The Stripe webhook handler at /api/stripe/webhook intermittently fails signature verification with 'No signatures found matching the expected signature for payload'. Root cause appears to be that Vercel's body parser re-encodes the raw body before the handler sees it.",
    reproSteps: [
      "Subscribe to a paid plan in production.",
      "Observe Stripe sends checkout.session.completed to /api/stripe/webhook.",
      "Server logs show 400 'signature verification failed'.",
      "Stripe retries the webhook 3 times, all fail.",
    ],
    expectedBehavior: "Webhook verifies the signature and processes the event.",
    actualBehavior: "All retries fail with 400. Subscription remains in trialing state.",
    productType: "non-ai",
    module: "Billing",
    environment: "production",
    environmentDetails: "Vercel Node.js runtime",
    methodologyId: "na-integration",
    methodologyCategory: "non-ai-strategies",
    severity: "critical",
    priority: "P0",
    status: "in_progress",
    resolution: "open",
    assigneeUserId: "user-vikram",
    assigneeName: "Vikram Shah",
    reporterUserId: "user-priya",
    reporterName: "Priya Menon",
    ccUserIds: ["user-arjun"],
    tags: ["stripe", "webhook", "billing", "vercel"],
    comments: [
      {
        id: "c-5",
        authorName: "Priya Menon",
        authorRole: "Founder & CEO",
        body: "Customer reported they paid but still see 'trialing'. Need this fixed today.",
      ...
      },
      {
        ...
        authorName: "Vikram Shah",
        authorRole: "AI QA Lead",
        body: "Reproduced in staging. Confirming raw body issue — switching to `export const runtime = 'nodejs'` and reading req.body via `await req.text()`.",
        changeKind: "status",
        changeFrom: "assigned",
        changeTo: "in_progress",
  ...
  },
  {
    id: "bug-4",
    displayId: "BUG-1004",
    title: "Image Studio download opens in new tab instead of triggering download",
    description:
      "On Safari (macOS 14) and Firefox 125, clicking 'Download PNG' in Image Studio opens the image in a new browser tab rather than triggering a file download. The download attribute on <a> is not respected for cross-origin blob URLs.",
    reproSteps: [
      "Open Image Studio on Safari 17 (macOS 14).",
      "Generate any image.",
      "Click 'Download PNG'.",
      "Image opens in a new tab; user must right-click → Save As.",
    ],
    expectedBehavior:
      "Clicking Download PNG should trigger a browser file download with the correct filename.",
    actualBehavior:
      "New tab opens with the image; no download is triggered. Same behavior on Firefox 125.",
    productType: "non-ai",
    module: "Image Studio",
    environment: "production",
    environmentDetails: "Safari 17 / macOS 14 · Firefox 125 / Windows 11",
    methodologyId: "na-compatibility",
    methodologyCategory: "non-ai-strategies",
    severity: "minor",
    priority: "P3",
    status: "fixed",
    ...
    tags: ["image-studio", "safari", "firefox", "download"],
    comments: [
      {
        id: "c-7",
        authorName: "Rohan Das",
        authorRole: "Social Media Manager",
        body: "Reported by 2 users. Chrome works fine.",
      ...
      },
      {
        ...
        authorName: "Vikram Shah",
        authorRole: "AI QA Lead",
        body: "Fixed in commit f65467a — convert base64 to Blob → object URL so the download attribute is honored. Will verify in next deploy.",
        changeKind: "status",
        changeFrom: "in_progress",
        changeTo: "fixed",
      ...
      },
      {
        ...
        authorName: "Rohan Das",
        authorRole: "Social Media Manager",
        body: "Verified on Safari 17 + Firefox 125 in production post-deploy. Working.",
        changeKind: "status",
        changeFrom: "fixed",
        changeTo: "verified",
        createdAt: "2026-07-04T09:30:00.000Z",
    ...
    attachments: [],
    createdAt: "2026-07-03T10:55:00.000Z",
    updatedAt: "2026-07-04T09:30:00.000Z",
    closedAt: "2026-07-04T10:00:00.000Z",
  },
  {
    id: "bug-5",
    displayId: "BUG-1005",
    title: "AI Testing report card score dropdown doesn't reflect methodology weight",
    description:
      "The AI Testing module's overall score is computed as a flat average across all categories, but the pass criteria for 'AI Model Validation' explicitly states that some categories should be weighted higher. The displayed score doesn't honor that weighting.",
    reproSteps: [
      "Open AI Testing module.",
      "Run a test on any AI chatbot tool with all 9 scenarios.",
      "Observe overall score = simple average of 9 category scores.",
    ],
    expectedBehavior:
      "Critical categories (chatbot-correctness, prompt-injection) should weight 2x; minor categories (recommendation-latency) weight 0.5x.",
    actualBehavior:
      "All 9 categories weight equally, producing a misleading overall score.",
    productType: "ai",
    module: "AI Testing",
    environment: "production",
    environmentDetails: "All browsers",
    methodologyId: "ai-model-validation",
    methodologyCategory: "ai-strategies",
    severity: "normal",
    priority: "P2",
    status: "new",
    resolution: "open",
    reporterUserId: "user-vikram",
    reporterName: "Vikram Shah",
    tags: ["ai-testing", "scoring", "weighting"],
    comments: [
      {
        id: "c-10",
        authorName: "Vikram Shah",
        authorRole: "AI QA Lead",
        body: "Spotted while reviewing the v3 report card. Need to add a weighting matrix to the scoring function.",
        createdAt: "2026-07-05T16:00:00.000Z",
      },
    ],
    attachments: [],
    createdAt: "2026-07-05T15:50:00.000Z",
    updatedAt: "2026-07-05T16:00:00.000Z",
  },
  {
    id: "bug-6",
    displayId: "BUG-1006",
    title: "Wiki search doesn't index the new Sales Agents Manual doc",
    description:
      "After adding the 'AI Sales Agents — Manual' wiki doc, the wiki search box returns zero results for terms that appear in that doc (e.g. 'BANT', 'MEDDIC', 'qualifier'). The search filters the DOCS list in-memory but only checks title + audience + description, not the body.",
    reproSteps: [
      "Open Wiki / Docs module.",
      "Type 'BANT' in the search box.",
      "Observe zero results.",
    ],
    expectedBehavior:
      "Search should surface 'AI Sales Agents — Manual' when the query matches content in the doc body.",
    actualBehavior: "Zero results returned.",
    productType: "non-ai",
    module: "Wiki / Docs",
    environment: "production",
    methodologyId: "na-functional",
    methodologyCategory: "non-ai-strategies",
    severity: "minor",
    priority: "P3",
    status: "unconfirmed",
    resolution: "open",
    reporterUserId: "user-isha",
    reporterName: "Isha Kapoor",
    tags: ["wiki", "search", "ux"],
    comments: [],
    attachments: [],
  ...
  },
  {
    id: "bug-7",
    displayId: "BUG-1007",
    title: "WhatsApp broadcast CSV import silently drops rows with international numbers",
    description:
      "When importing a CSV of WhatsApp contacts, rows with phone numbers in E.164 format starting with '+' are silently dropped. The parser appears to choke on the leading '+' and treats the row as malformed.",
    reproSteps: [
      "Open WhatsApp Marketing module.",
      "Import a CSV where the phone column contains values like '+14155551234'.",
      "Observe the imported count is lower than the CSV row count.",
    ],
    expectedBehavior:
      "All rows with valid E.164 numbers should be imported. The '+' prefix is part of the standard.",
    actualBehavior:
      "Rows with leading '+' are dropped silently — no error, no warning.",
    productType: "non-ai",
    module: "WhatsApp Marketing",
    environment: "production",
    methodologyId: "na-data-validation",
    methodologyCategory: "non-ai-methodologies",
    severity: "major",
    priority: "P1",
    status: "new",
    resolution: "open",
    reporterUserId: "user-meera",
    reporterName: "Meera Nair",
    tags: ["whatsapp", "csv-import", "data-validation"],
    comments: [
      {
        id: "c-11",
        authorName: "Meera Nair",
        authorRole: "Email Marketer",
        body: "Lost 200+ international contacts in last import.",
